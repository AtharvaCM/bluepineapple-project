...
function CricRankBatting() {
    ...
    useEffect(() => {
        CricRankTestApi().then((response) => {setDetails(response.data)}
    ...
    const changeData = (Data) => {
        if (Data === "Test") {
            ...
            CricRankOdiApi().then((response) => {setDetails(response.data)}
            ).catch((err) => console.log(err));
        }
        else if (Data === "T20") {
            CricRankT20Api().then((response) => {setDetails(response.data)}
            ).catch((err) => console.log(err));
        }
    }
    return (
                            ...
                            {Details.map(player => {
                                return (
                                    ...
                                    <tr key={player.id}>
                                        ...
                                        <td ><h4 onClick={() => routeChange(player)}>{player.name}</h4></td>
                                    </tr>
                                )
                            }
                            )}
